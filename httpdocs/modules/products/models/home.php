...
//        var_dump($linhvuc);
        ...
        if ($linhvuc) {
            ...
            foreach ($lvs as $lv) {
                if ($lv == reset($lvs)) {
                    $where1 .= " category_alias like '%" . $lv . "%'";
                } else {
                    $where1 .= " OR category_alias like '%" . $lv . "%'";
                }
            }
            $where1 .= " )";
        }
        if ($hangsx) {
            ...
            foreach ($sxs as $sx) {
                if ($sx == reset($sxs)) {
                    $where1 .= "  manufactory_alias like '%" . $sx . "%'";
                } else {
                    $where1 .= " OR manufactory_alias like '%" . $sx . "%'";
                }
            }
            $where1 .= " )";
        }
        if ($loaisp) {
            ...
            foreach ($sps as $sp) {
                if ($sp == reset($sps)) {
                    $where1 .= " types_alias like '%" . $sp . "%'";
                } else {
                    $where1 .= " OR types_alias like '%" . $sp . "%'";
                }
            }
            $where1 .= " )";
        ...
        if ($ungdung) {
            ...
            foreach ($uds as $ud) {
                if ($ud == reset($uds)) {
                    $where1 .= " application_alias like '%" . $ud . "%'";
                } else {
                    $where1 .= " OR application_alias like '%" . $ud . "%'";
                }
            }
            $where1 .= " )";
        }
        ...
        global $db;
        $query = "SELECT count(*)  FROM $this->table_name WHERE published = 1" . $where1 . $where2 . $where;
        $db->query($query);
        $total = $db->getResult();
        return $total;
    }
...
//        var_dump($order);
        ...
        $query = "SELECT * FROM $this->table_name WHERE published = 1 and is_hot = 1" . $where1 . $where2  . " order by RAND()";
        if ($order == 'a_z') {
            $query .= ", name ASC";
        }
        $query .= " LIMIT 9";
...
//        echo $query = "SELECT * FROM fs_products WHERE published = 1".$where1.$where2.$where;
        $query = "SELECT * FROM $this->table_name WHERE published = 1" . $where1 . $where2 . $where;
    ...
    {
        $query = "SELECT * FROM $this->table_manu WHERE published = 1  AND ordering != 3000 ORDER BY name ASC";
        global $db;
        $db->query($query);
        $result = $db->getObjectList();
        return $result;
    }
    function getproducts_manufactories_other()// không sử dụng limit để show danh sách
    {
        $query = "SELECT * FROM $this->table_manu WHERE published = 1  AND ordering = 3000";
        global $db;
        $db->query($query);
        $result = $db->getObjectList();
        return $result;
