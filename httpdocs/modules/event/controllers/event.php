 ...
 */
	...
	class EventControllersEvent extends FSControllers
	{
		...
		function display()
		{
			...
			// print_r($sukiennoibat);
			
//			 var_dump($tinchitiet);
			// $tinlienquan = $model->gettinlienquan($tinchitiet->category_id);
...
//			}
            $link = FSRoute::_('index.php?module=event&view=event&code=' . $data->alias . '&id=' . $data->id);
...
//			var_dump($tag1);die;
			...
			$breadcrumbs[] = array(0=>$category -> name, 1 => FSRoute::_('index.php?module=event&view=cat&id='.$data -> category_id.'&ccode='.$data -> category_alias));
