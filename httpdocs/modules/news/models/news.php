...
//		}
		...
		function getNews()
		{
			...
			$query = " SELECT id,title,content,category_id,category_alias, summary,hits,
                        alias, created_time, updated_time,seo_title,image,optimal_seo,
                        seo_keyword,seo_description,tags, news_related,products_related
...
// print_r($products_related);die;
    	...
    	function get_news_related($news_related, $news_id) {

			if (!$news_related || !$news_id)
    			return;
    		$limit = 20;
    		$rest_news_related_ = $news_related;  // retourne "abcde"
//			var_dump($rest_news_related_); die;
		...
		function gettinchitiet()// mẫu hàm để gọi chính xác 1 mảng
		{
			$id = FSInput::get('id');
				if(!$id)
					die('Not exist this url');
