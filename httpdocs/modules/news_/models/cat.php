...
//			$db->query($query);
    ...
    {
        $query = "SELECT * FROM " . $this->table_cat . " WHERE published = 1 AND parent_id = 0  ORDER BY ordering ASC";
        global $db;
        $db->query($query);
        $result = $db->getObjectList();
        return $result;
    }
    function getdanhmuc1()// không sử dụng limit để show danh sách
    {
        $query = "SELECT * FROM ".$this->table_cat." WHERE published = 1 AND parent_id = 11 ORDER BY ordering ASC";
        global $db;
        $db->query($query);
        $result = $db->getObjectList();
        return $result;
