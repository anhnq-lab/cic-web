<?php 
	class NewsModelsHome extends FSModels
	{
		...
		function get_ds_news()// mẫu hàm phân trang
		{
			global $db;// bắt buôc phải khai báo để kết nói với data
			$query = 'SELECT * FROM '.$this->table_name.' WHERE published = 1';// câu lệnh query truyền vào

			$sql = $db->query_limit($query,$this->limit,$this->page); // đây là câu lệnh biến đổi để phân trang

			$result = $db->getObjectList();// thực thi câu lệnh
