    ...
    CKFinder.popup( {
        ...
        onInit: function( finder ) {
            ...
            finder.on( 'files:choose', function( evt ) {
                ...
                var ext = file.getUrl().split('.').pop();
                $("#" + elementId).prev().empty();
                if(ext == 'png' || ext == 'jpeg') {
                    $("#" + elementId).prev().append(`<img style="width: 100px; height: 100px; object-fit: contain" src="${file.getUrl()}" />`)
                }
                if(ext == 'mp3' || ext == 'ogg') {
                    $("#" + elementId).prev().append(`<audio controls><source src="${file.getUrl()}" type="audio/ogg"></audio>`)
                }
                if(ext == 'mp4') {
                    $("#" + elementId).prev().append(`<video width="320" height="240" controls><source src="${file.getUrl()}" type="audio/ogg"></video>`)
                }
...
// check menu hie show admin
$( document ).ready(function() {
    $('#sidebar-menu li.active').parent('ul').parent('li').addClass('menu-open active');
    $('#sidebar-menu li.active').parent('ul').show();
...
$('#example2').DataTable({
  ...
  'autoWidth'   : false,
  "language": {
            "lengthMenu": "Hiển thị _MENU_ trên mỗi trang trang",
            "zeroRecords": "Không có dữ liệu",
            "info": "Ðang ở trang _PAGE_ của _PAGES_",
