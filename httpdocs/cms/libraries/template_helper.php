...
class TemplateHelper
{
    ...
    public static function jscolorpicker($title, $name, $value, $default = 'FFFFFF', $size = 10, $comment = '', $class_col1 = 'col-md-3', $class_col2 = 'col-md-9')
    {
        ...
        echo '<div class="form-group">';
        echo '	<label>' . $title . '</label>';
        echo '	<div class="input-group my-colorpicker ">';
        echo '		<input class="form-control" type="text" name="' . $name . '"  id="' . $name . '" value="' . htmlspecialchars($value) . '" size="' . $size . '"/>';
        ...
        echo '	</div>';
        echo '';
        echo '<script>$(".my-colorpicker").colorpicker();</script>';
        echo '</div>';
    }
    ...
    {
        $html = '<a title="Views" href="' . $link . '" style="font-size:20px;">';
    ...
    public static function genarate_form_liting($module, $view, $list, $fitler_config, $list_config, $sort_field, $sort_direct, $pagination = null, $params = array(), $first = 'index.php')
    {
        ...
        $html_begin = '<di class="col-xs-12"><form class="box box-primary" action="' . $link . '" name="adminForm" method="post">
        ...
        $html_head = '<div class="dataTable_wrapper">
                        <table style="width: 100%;" id="example2" class="table table-hover table-striped table-bordered display responsive nowrap" style="width:100%">
                        <thead>
                            <tr>';
            ...
            foreach ($list as $row) {
                ...
                foreach ($arr_config as $col) {
                    ...
                    foreach ($col as $item) {
                        ...
                        $field_chiden = isset($item['field']) ? $item['field'] : '';
                        $field_curent = $field_chiden ? $row->$field_chiden : '';
                        switch ($type) {
                            case 'published':
                            ...
                            case 'change_status':
                                ...
                                if ($inverse == 1)
                                    $html_body .= TemplateHelper::changeStatus("cb" . ($i), $field_curent ? $function : "un" . $function);
                                else
                                    $html_body .= TemplateHelper::changeStatus("cb" . ($i), $field_curent ? "un" . $function : $function);
                                break;
                            case 'edit':
                                $link_edit = "index.php?module=" . $module . "&view=" . $view . "&task=edit&id=" . $row->id . "&page=" . $page;
                            ...
                            case 'datetime':
                                if (!$field_curent)
                                    ...
                                    $html_body .= date('d/m/Y', strtotime($field_curent));
                                break;
                            case 'date':
                                $html_body .= date('d/m/Y', strtotime($field_curent));
                                break;
                            case 'timestamp':
                                $html_body .= date('d/m/Y', $field_curent);
                                break;
                            case 'edit_text':
                                $size = isset($item['arr_params']['size']) ? $item['arr_params']['size'] : 10;
                                $rows = isset($item['arr_params']['rows']) ? $item['arr_params']['rows'] : 1;
                                $html_body .= TemplateHelper::edit_text($item['field'], $field_curent, $i, $size, $rows);
                                break;
                            case 'text_status':
                                $module = FSInput::get('module');
                            ...
                            case 'edit_selectbox':
                                ...
                                $html_body .= TemplateHelper::edit_selectbox($item['field'], $field_curent, $i, $arry_select, $field_value, $field_label, $size, $multi);
                                break;
                            case 'image':
                                $link_img = $field_curent;
                            ...
                            case 'order':
                                ...
                                break;
                            case 'format_money':
                                $html_body .= format_money_0($field_curent, '', '');
                                break;
                            case 'log':
                            ...
                            case 'text_link':
                                ...
                                if ($link_web) {
                                    ...
                                    $html_body .= '<a style="text-align: left;max-width : 100px" target="_blank" href="' . $link_web . '">' . $field_curent . '</a>';
                                }
                                break;
                            case 'text_link1':
                                $link = '';
                                ...
                                $html_body .= '<a style="text-align: left;max-width : 100px" target="_blank" href="' . $field_curent . '">' . $link . '</a>';
                                break;
                            case 'table_name':
                                $table_name = $field_curent;
                                ...
                                break;
                            case 'status':
                                $status = isset($field_curent) ? 1 + $field_curent : '';
                                if ($status) {
                                ...
                                if (isset($item['arr_params']['function']) && !empty($item['arr_params']['function'])) {
                                    ...
                                    if ($item['field']) { // nếu định nghĩa field: params sẽ truyền vào là $row -> field
                                        if ($params_for_function)
                                            $html_body .= $this->$function($field_curent, $params_for_function, $i);
                                        //else 
                                    ...
                                    if (isset($item['arr_params']['have_link_edit']) && !empty($item['arr_params']['have_link_edit']))
                                        $html_body .= '<a href="' . $link_edit . '" >' . $field_curent . '</a>';
                                    else
                                        $html_body .= $field_curent;
                                    break;
                                }
                        }
                        $j++;
        ...
        $html = $html_begin . $html_head . $html_body . $html_footer . '</div></form>';
        $html .= '<script>
...
//                document.adminForm.submit();
    ...
    static function dt_edit_selectbox($title, $name, $value, $default = '', $arry_select = array(), $field_value = 'id', $field_label = 'name', $size = 1, $multi = 0, $add_fisrt_option = 0, $comment = '', $sub_item = '', $class = 'chosen-select chosen-select-deselect', $class_col1 = 'col-md-3', $class_col2 = 'col-md-9', $field = '')
    {
        ...
        $class = $class ? $class : 'select2';
        $html = '<div class="form-group">
                    <label>' . $title . '</label>
                    ';
        if (!$multi) {
            $html_sized = $size > 1 ? "size=$size" : "";
            $html .= '<select data-placeholder="' . $title . '" class="select2 ' . $class . '" name="' . $name . '" id="' . $name . '" ' . $html_sized . '>';
            $compare = 0;
            if (isset($value))
                $compare = $value;
            else
            ...
            // not working
            $html_sized = $size > 1 ? "size=$size" : "";
            $html .= '<select data-placeholder="' . $title . '" class="select2" name="' . $name . '[]" id="' . $name . '" ' . $html_sized . ' multiple="multiple">';
            //  if($add_fisrt_option){
            ...
            $html .= '</select>';
            $html .= '<a class="btn btn-primary chosen-toggle select" style="cursor: pointer;margin-top: 10px;margin-right: 10px;">' . FSText::_('chọn tất cả') . '</a>';
            $html .= '<a class="btn btn-danger chosen-deselect deselect" style="cursor: pointer;margin-top: 10px;">' . FSText::_('Xóa chọn') . '</a>';
        }
        if ($sub_item)
            $html .= $sub_item;
        if ($comment)
        ...
        $html .= '<script type="text/javascript">
                        ...
                        $(".chosen-deselect").each(function(index) {
                            $(this).on("click", function(){
    ...
    public static function dt_edit_choose_file($title, $name, $value = '', $comment = '', $class_col1 = 'col-md-3', $class_col2 = 'col-md-9', $id_element = null)
    {
        // phan quyen theo field
        $module = FSInput::get('module');
        $view = FSInput::get('view', $module);
        ...
        $check_val = $value ? 1 : 0;
        $id_element = $id_element ? $id_element : $name;
        $filename = $value;
        $extension = pathinfo($filename, PATHINFO_EXTENSION);

        $html .= '<div class="box-choose-file">';
        if ($extension == 'mp3') {
            $html .= '<audio controls><source src="' . $value . '" type="audio/ogg"></audio>';
        }
        if ($extension == 'jpg' || $extension == 'jpeg' || $extension == 'png') {
            $html .= '<img style="width: 100px; height: 100px; object-fit: contain" src="' . $value . '" />';
        }
        if ($extension == 'mp4') {
            $html .= '<video width="320" height="240" controls><source src="' . $value . '" type="audio/ogg"></video>';
        }
        $html .= '</div>';
        $html .= '<input type="text" class="form-control" name="' . $name . '" id="' . $id_element . '" value="' . $value . '" size="60" maxlength="60">
        <a id="btn-video-' . $id_element . '" style="color: white !important; margin-top: 10px" class="btn btn-primary" href="javascript:void(0)">Chọn tệp</a>
        ';

        $html .= '
        <script type="text/javascript">
            $("#' . $id_element . '").on("input", function() {});
            $("#btn-video-' . $id_element . '").click(function () {
    ...
    public static function create_filter($row, $prefix)
    {
        ...
        if (isset($row['search'])) {
            ...
            $html .= '</div>';
            $html .= $html_text;
            $html .= '<div class="col-xs-12 col-sm-1 form-fillter">
                        ';

            //$html .= '<div class="col-sm-6 col-xs-12">
            //                        <button class="btn btn-primary col-xs-12" onclick="this.form.submit();">' . FSText :: _('Search') . '</button>
            //                    </div>';
            $html .= '
                        <button class="btn btn-default col-xs-12" onclick="document.getElementById(\'search\').value=\'\';';
            if (isset($row['text_count'])) {
                $count = $row['text_count'];
                for ($i = 0; $i < $count; $i++) {
            ...
            $html .= '				this.form.getElementById(\'filter_state\').value=\'\';this.form.submit();">' . FSText::_('Reset') . '</button></div>';
            //$html .= '		</div>';
        }
        ...
        if (isset($row['filter_count'])) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($type == 'yesno') {
                    ...
                    $html .= '				<select name="filter' . $i . '" class="select2 ' . $class . '" onChange="this.form.submit()">';
