...
//			$this -> img_folder = '';
			$this -> load_params();
            $this->table_category = FSTable_ad::_('fs_products_categories');
		}
...
//				}
		...
		function save($row = array(),$use_mysql_real_escape_string = 1){
			...
			if(!$id){
				$id = $this -> save_new($row,$use_mysql_real_escape_string);
                if($id && $this->type){
                    ...
                    $row['session_id'] = '';
                    $row['record_id'] = $id;
                    $this->_update($row,'fs_'.$this->type.'_images',' session_id = "'.$data[1].'"');
                }
                return $id;
			}else{ 
		...
		function save_new($row,$use_mysql_real_escape_string = 0){
			...
			$str_fields = array();
			$str_values = array();
			$field_img = !empty($this -> field_img)?$this -> field_img:'image';
			$field_width = !empty($this -> field_width)?$this -> field_width:'';
			
			for($i = 0; $i < count($fields_in_table); $i ++){
				$item = $fields_in_table[$i];
				$field  = $item -> Field;
				if($field == $field_img && !isset($row[$field_img]) && $field_img ){
					$image = !empty($_FILES[$field_img]["name"])? $_FILES[$field_img]["name"]:'';
					if($image){
						$image = $this -> upload_image($field_img,'_'.time(),3000000);
						$row[$field_img] = 	$image;
				...
				if($field == 'alias'){
					...
					if(isset($this -> check_alias) && $this -> check_alias){
						if($this -> check_exist($row['alias'],0)){
							Errors::_('Alias của bạn đã bị trùng tên','alert');
							$row['alias'] = $this -> genarate_alias_news($row['alias'],0);
						}
					}
				}
				if(isset($row[$field])){
...
//			var_dump($fields_in_table);die;
			...
			for($i = 0; $i < count($fields_in_table); $i ++){
				...
				$field  = $item -> Field;
                if($field == $field_img && !isset($row[$field_img]) && $field_img){
                    $field_img_real= FSInput::get($field_img);
                    if ($field_img_real){
...
//					$row['tablename']  = $filter_current -> tablename;
		 ...
		 * (insert or update)
		 */
		function save_extension_exel($tablename, $record_id = 0, $row_exel) {
  ...
  //   			return false;
	...
	function change_title_attr_image() {
		...
		function get_product_categories_tree_by_permission()
		{
			...
			global $db;
			$query = ' SELECT *
	                   FROM '.$this->table_category.' WHERE 1 = 1 '.$where.'
                       ORDER BY ordering ';
			$sql = $db->query($query);
			$result = $db->getObjectList();
			$tree  = FSFactory::getClass('tree','tree/');
