<?php 
	class Config_enjicadModelsConfig_enjicad   extends FSModels
	{

		function __construct()
		{
			parent::__construct();
            $this -> name_table = FSTable_ad::_('fs_config_enjicad');
		}
			...
			foreach($data as $item)
			{
				if($item->data_type == 'editor')
					...
					if(isset($_FILES[$item->name]['name']) && !empty($_FILES[$item->name]['name'])){
						$path = PATH_BASE.'images'.DS.'config_enjicad'.DS;
						$image = $fsFile -> uploadImage($item->name, $path,2000000,'_'.time());
						if(!$image)	
							continue;
						$value = 'images/config_enjicad/'.$image;
					}else{
						continue;
					}
				} else 
