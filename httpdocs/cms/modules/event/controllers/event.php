<?php
	class EventControllersEvent extends Controllers
	{
		...
		function edit()
		{
			...
			$categories_products = $model->get_categories_tree2();



            // print_r($categories_products);die;
...
//           var_dump($news_related);die;
        ...
        function ajax_get_event_related(){
			$model = $this -> model;
			$data = $model-> ajax_get_event_related();
            $str_related = FSInput::get('str_related');
            $id = FSInput::get('id',0,'int');
			$html = $this -> event_genarate_related($data,$str_related,$id);
			echo $html;
			return;
		}
        function ajax_get_products_related(){
            ...
            $str_related = FSInput::get('str_related');
            $id = FSInput::get('id',0,'int');
			$html = $this -> products_genarate_related($data,$str_related,$id);
			echo $html;
			return;
		}
        function products_genarate_related($data,$str_related=0,$id){
        ...
        function event_genarate_related($data,$str_related=0,$id){
			$str_exist = FSInput::get('str_exist');
            $error_img = "this.src='/images/1443089194_picture-01.png'";
            if($str_related){
                if($id){
			...
			$html = '';
				$html .= '<div class="event_related">';
				foreach ($data as $item){
					if($str_exist && strpos(','.$str_exist.',', ','.$item->id.',') !== false ){
						$html .= '<div class="red event_related_item  event_related_item_'.$item -> id.'" onclick="javascript: set_event_related('.$item->id.')" style="display:none" >';	
						$html .= '<strong>'.$item -> title.'</strong><br/>';
						$html .= '<img onerror="'.$error_img.'" src="'.str_replace('/original/','/small/',URL_ROOT.@$item->image).'">';				
						$html .= '</div>';					
					}else{
						$html .= '<div class="news_related_item  event_related_item_'.$item -> id.'" onclick="javascript: set_event_related('.$item->id.')">';
                        $html .= '<strong>'.$item -> title.'</strong><br/>';
						$html .= '<img onerror="'.$error_img.'" src="'.str_replace('/original/','/small/',URL_ROOT.@$item->image).'">';		
						$html .= '</div>';	
					}
