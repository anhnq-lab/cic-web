	...
	class EventModelsEvent extends FSModels
	{
		...
		function __construct()
		{
			...
			$this -> arr_img_paths = array(
                                            array('resized',720,405,'resize_image'),
                                            array('small',260,150,'resize_image'),
                                            // array('small',100,60,'resize_image'),
                                           array('large',580,345,'resize_image')
                                        );
			$this -> table_category_name = FSTable_ad::_('fs_news_categories');
            $this -> table_name = FSTable_ad::_('fs_event',1);
            $this -> table_news = FSTable_ad::_('fs_news',1);
            $this -> table_link = 'fs_menus_createlink';
            $this -> table_products = 'fs_products';
            $this -> table_products_categories = 'fs_products_categories';
...
//					$this -> remove_file($id,$img_paths,'file_upload');
                ...
                // upload
                $file_upload_name = $fsFile->upload_file("file_upload", $path_original, 500000000, '_' . time());
                if (!$file_upload_name)
                    return false;
...
//    		 	var_dump($row['news_related']);die;
			 }

			$record_relate_news = FSInput::get('news_record_related',array(),'array');
			$row['news_related'] ='';
    		 if(count($record_relate_news)){
    		 	$record_relate_news = array_unique($record_relate_news);
    		 	$row['news_related'] = implode(',', $record_relate_news);
//    		 	var_dump($row['news_related']);die;
			 }

			$record_relate_products = FSInput::get('products_record_related',array(),'array');
			$row['products_related'] ='';
    		 if(count($record_relate_products)){
    		 	$record_relate_products = array_unique($record_relate_products);
    		 	$row['products_related'] = implode(',', $record_relate_products);
//    		 	var_dump($row['news_related']);die;
			 }
			...
			$time1 =  FSInput::get('created_time');
//            $row['created_time'] = $time1;
//            $row['published'] = 1;
        ...
        function get_event_related($event_related){
			// echo 1;die;
    		...
    		$query   = " SELECT id, title,image 
    					...
    					WHERE id IN (".$event_related.") 
    					 ORDER BY POSITION(','+id+',' IN '".$event_related."')
    					";
   		// print_r($query);die;
    		global $db;
    		$sql = $db->query($query);
    	...
    	// 				WHERE id IN (".$news_related.") 
    	// 				 ORDER BY POSITION(','+id+',' IN '".$news_related."')
    	// 				";
   		// // print_r($query);die;
    	// 	global $db;
    	// 	$sql = $db->query($query);
    	...
    	function ajax_get_event_related(){
...
//                        $where .= " title LIKE '%".$keyword_tag[$i]."%' OR alias LIKE '%".$keyword_tag[$i]."%' OR content LIKE '%".$keyword_tag[$i]."%' ";
                        $where .= " title LIKE '%".$keyword_tag[$i]."%' OR alias LIKE '%".$keyword_tag[$i]."%' ";
                    }else{
//                       $where .= " OR title LIKE '%".$keyword_tag[$i]."%' OR alias LIKE '%".$keyword_tag[$i]."%' OR content LIKE '%".$keyword_tag[$i]."%' ";
                       $where .= " OR title LIKE '%".$keyword_tag[$i]."%' OR alias LIKE '%".$keyword_tag[$i]."%'";
                    }
                }
                $where .= ' ) '; 
            }
...
//    		$query = ' SELECT id,category_id,title,category_name,image'.$query_body.$ordering.' LIMIT 100 ';
    		$query = ' SELECT id,category_id,title,category_name,image'.$query_body.$ordering.' ';
            //print_r($query);
    		global $db;
    		$sql = $db->query($query);
    		$result = $db->getObjectList();
...
//                       $where .= " OR title LIKE '%".$keyword_tag[$i]."%' OR alias LIKE '%".$keyword_tag[$i]."%' OR content LIKE '%".$keyword_tag[$i]."%' ";
    		...
    		$ordering = " ORDER BY created_time DESC , id DESC ";
//    		$query = ' SELECT id,category_id,title,category_name,image'.$query_body.$ordering.' LIMIT 100 ';
    		$query = ' SELECT id,category_id, name,category_name,image'.$query_body.$ordering.' ';
            // print_r($query);
    		global $db;
    		$sql = $db->query($query);
    		$result = $db->getObjectList();
...
//    		var_dump($query);die;
		...
		function get_products_related($products_related){
    		if(!$products_related)
    				return;
    		$query   = " SELECT id, title,image ,name
    					FROM ".$this -> table_products."
    					WHERE id IN (".$products_related.") 
    					 ORDER BY POSITION(','+id+',' IN '".$products_related."')
    					";
//    		var_dump($query);die;
    		global $db;
    		$sql = $db->query($query);
...
//    		$query = ' SELECT id,category_id,title,category_name,image'.$query_body.$ordering.' LIMIT 100 ';
		...
		function get_categories_tree2()
		{
			...
			$query = " SELECT a.*
						  FROM 
						  	".$this -> table_products_categories." AS a
						  	WHERE published = 1 ORDER BY ordering ";         
			$sql = $db->query($query);
			$result = $db->getObjectList();
			$tree  = FSFactory::getClass('tree','tree/');
