<div class="products_related">
	<div class="row">
		...
		<div class="col-xs-12 col-md-6">
			<div class='title-related'><?php echo FSText::_('Danh sách sản phẩm liên quan'); ?></div>
			<div id='products_related_l'>
				<div id='products_related_search_list'>
					<input type="hidden" value="<?php echo @$data->products_related ?>" id="str_related" name="str_related" />
				</div>
			</div>
		</div>
		...
		<div class="col-xs-12 col-md-6">
			<div id='products_related_r'>
				<!--	LIST RELATE			-->
				<div class='title-related'><?php echo FSText::_('Sản phẩm liên quan'); ?></div>
				<ul id='products_sortable_related'>
					...
					// print_r($item);
					?>
						<li id='products_record_related_<?php echo $item->id ?>'><strong><?php echo $item->name; ?></strong>
							<a class='products_remove_relate_bt' onclick="javascript: remove_products_related(<?php echo $item->id ?>)" href="javascript: void(0)" title='Bỏ chọn'>
								<img border="0" alt="Remove" src="libraries/uploadify/delete.png">
							</a>
							<br />
							<strong></strong>

							<img width="80" src="<?php echo URL_ROOT . str_replace('/original/', '/original/', $item->image); ?>">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		if (keyword_tag) {
			$("#products_related_search_list").load("index2.php?module=event&view=event&task=ajax_get_products_related&raw=1", {
		...
		$('textarea#tags').on('change', function() {
			...
			$.ajax({
				type: 'POST',
				url: 'index2.php?module=event&view=event&task=ajax_get_products_related&raw=1',
				data: {
					keyword_tag: keyword_tag,
					str_related: str_related
				},
				...
				success: function(data) {
					$('#products_related_search_list').html(data);
				},
				error: function() {
					// code here
				}
	...
	function search_products_related() {
		$('#products_related_search').click(function() {
			...
			$.get("index2.php?module=event&view=event&task=ajax_get_products_related&raw=1", {
				new_id: new_id,
	...
	function set_products_related(id) {
		...
		var name = $('.products_related_item_' + id).html();
		var html = '<li id="products_record_related_' + id + '"><strong>' + name + '</strong><input type="hidden" name="products_record_related[]" value="' + id + '" />';
		html += '<a class="products_remove_relate_bt"  onclick="javascript: remove_products_related(' + id + ')" href="javascript: void(0)" title="Xóa"><img border="0" alt="Remove" src="libraries/uploadify/delete.png"></a>';
		html += '<hr></li>';
