...
class FSFiles
{
	 ...
	 */
	function uploadImage($inputTagName, $path, $sizeMax = 2000000, $suffix = '', $crop = 0, $cropWidth = 0, $cropHeight = 0) {
    if (!isset($_FILES[$inputTagName])) {
        Errors::setError("File input not found.");
        return false;
    }
    
    $fileType = $_FILES[$inputTagName]["type"];
    $allowedTypes = ["image/gif", "image/png", "image/jpeg", "image/pjpeg"];
    
    if (!in_array($fileType, $allowedTypes)) {
        Errors::setError("Extension of file is not valid.");
        return false;
    }
    
    if ($_FILES[$inputTagName]["size"] > $sizeMax) {
        Errors::setError("File size exceeds the allowed limit.");
        return false;
    }
    
    if ($_FILES[$inputTagName]["error"] > 0) {
        Errors::setError("File error detected.");
        return false;
    }
    
    $fileNew = $_FILES[$inputTagName]["name"];
    $fileExt = pathinfo($fileNew, PATHINFO_EXTENSION);
    $fileName = pathinfo($fileNew, PATHINFO_FILENAME);
    
    $fileName = preg_replace("/[^a-zA-Z0-9_-]/", "", $fileName);
    
    if (trim($suffix) !== '') {
        $fileNew = $fileName . $suffix . "." . $fileExt;
    } else {
        $fileNew = $fileName . "." . $fileExt;
    }
    
    if (file_exists($path . $fileNew)) {
        Errors::setError("File already exists: " . $fileNew);
        return false;
    }
    
    if (!is_dir($path) && !mkdir($path, 0777, true)) {
        Errors::setError("Failed to create directory: " . $path);
        return false;
    }
    ...
    if ($crop && $cropWidth && $cropHeight) {
        list($oldWidth, $oldHeight) = getimagesize($_FILES[$inputTagName]["tmp_name"]);
        $newWidth = $cropWidth;
        $newHeight = $cropHeight;
        
        $croppedTmp = imagecreatetruecolor($newWidth, $newHeight);
        
        switch ($fileExt) {
            case "png":
                $source = imagecreatefrompng($_FILES[$inputTagName]["tmp_name"]);
                break;
            case "jpg":
            case "jpeg":
                $source = imagecreatefromjpeg($_FILES[$inputTagName]["tmp_name"]);
                break;
            case "gif":
                $source = imagecreatefromgif($_FILES[$inputTagName]["tmp_name"]);
                break;
            default:
                Errors::setError("Unsupported image format.");
                return false;
        }
        
        if (!imagecopyresampled($croppedTmp, $source, 0, 0, 0, 0, $newWidth, $newHeight, $oldWidth, $oldHeight)) {
            Errors::setError("Failed to crop image.");
            return false;
        }
        
        switch ($fileExt) {
            case "png":
                imagepng($croppedTmp, $path . $fileNew);
                break;
            case "jpg":
            case "jpeg":
                imagejpeg($croppedTmp, $path . $fileNew, 90);
                break;
            case "gif":
                imagegif($croppedTmp, $path . $fileNew);
                break;
        }
        
        imagedestroy($croppedTmp);
        imagedestroy($source);
    } else {
        if (!move_uploaded_file($_FILES[$inputTagName]["tmp_name"], $path . $fileNew)) {
            Errors::setError("Failed to move uploaded file.");
            return false;
        }
    }
    
    chmod($path . $fileNew, 0777);
    return $fileNew;
}
...
//		if($file_ext == 'rar' || $file_ext == 'zip' || $file_ext == 'doc' || $file_ext == 'docx' || $file_ext == 'pdf')
	...
	function upload_media($input_tag_name, $path,$size_max= 2000000, $suffix){
		$file_new = $_FILES[$input_tag_name]["name"];
		$file_ext = $this -> getExt($_FILES[$input_tag_name]["name"]);
		if($file_ext == 'flv' || $file_ext == 'swf' || $file_ext == 'mp4')
			return $this->upload_file($input_tag_name, $path ,$size_max, $suffix);
		else{
			Errors:: setError('Extension of file is not format. Only user extension: .flv,.swf,.mp4');
			return false;
		}
	}
...
function add_logo($path,$filename,$filelogo = 'images/logo-site.png',$vitri=9,$text=''){
		...
		if(file_exists($filelogo)){
			...
			if($insertLogo==1){
				...
				case "jpg" :
					...
					break;
				case "gif" :
					@imagegif($image, $path . $filename,90);
					break;
				case "png" :
					@imagepng($image, $path . $filename,8);
					break;
				}
				imagedestroy($image);
			}
