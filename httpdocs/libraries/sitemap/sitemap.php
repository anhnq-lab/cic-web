...
//    }
    ...
    private function getDetails()
    {
        $details = '<url>
                      <loc>' . URL_ROOT . '</loc>
                      <lastmod>2018-12-03T05:24:25+00:00</lastmod>
...
//	}
    ...
    private function getItems_products()
    {
        ...
        $query = ' SELECT id,name,alias,created_time,edited_time
				FROM fs_products
				WHERE published = 1
				ORDER BY ID DESC
...
//        $db->query($query);
        ...
        for ($i = 0; $i < count($result); $i++) {
            $row = $result[$i];
            $link = FSRoute::_('index.php?module=products&view=product&code=' . $row->alias . '&id=' . $row->id);
            $xml .= '
                      <url>
                          <loc>' . $link . '</loc>
...
//        $db->query($query);
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            $link = FSRoute::_('index.php?module=products&view=cat&cid=' . $row->id . '&ccode=' . $row->alias);
            $xml .= '
                      <url>
                          <loc>' . $link . '</loc>
                          <lastmod>' . date("Y-m-d", strtotime($row->updated_time)) . 'T' . date("H-i-s", strtotime($row->updated_time)) . '+00:00</lastmod>
